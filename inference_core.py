...
# Optional cloudinary imports (only needed for cloud upload functionality)
try:
    import cloudinary
    ...
    print("[WARNING] Cloudinary not available. Cloud upload functions will not work.")

# ---- Configuration and model setup ----
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
CONFIG_PATH = os.path.join(BASE_DIR, "configs", "patchcore_transformers.yaml")
...
# ---- Output directories ----
OUT_MASK_DIR = os.path.join(BASE_DIR, "output_image", "masks")
OUT_FILTERED_DIR = os.path.join(BASE_DIR, "output_image", "filtered")  
OUT_BOXED_DIR = os.path.join(BASE_DIR, "output_image", "labeled")
...
if not os.path.exists(CKPT_PATH):
    raise FileNotFoundError(f"Model checkpoint not found at {CKPT_PATH}. Please ensure the model file exists.")
else:
    print(f"[INFO] Model checkpoint found at {CKPT_PATH}")
...
def upload_to_cloudinary(file_path, folder=None):
    """Upload file to Cloudinary (requires cloudinary package)"""
    if not CLOUDINARY_AVAILABLE:
        raise ImportError("Cloudinary package not available. Install with: pip install cloudinary")
